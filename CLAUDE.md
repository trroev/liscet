...
- **Runtime server secrets** (`DATABASE_URL`, `PAYLOAD_SECRET`, `BETTER_AUTH_SECRET`/`URL`, `BLOB_READ_WRITE_TOKEN`, `BLOB_PRIVATE_READ_WRITE_TOKEN`, `RESEND_*`, `CRON_SECRET`, `REVALIDATION_SECRET`, `VIRUS_SCAN_*`) → set as **native Vercel env vars** (Production + Preview) **and** keep in the encrypted `.env` files for local dev, CI, and the build-time migrate step. Media uses **two Blob stores**: a **public** store (`BLOB_READ_WRITE_TOKEN`) for the Payload `vercelBlobStorage` adapter (avatars, `/admin` media, SEO OG images — the adapter is public-only) and a **private** store (`BLOB_PRIVATE_READ_WRITE_TOKEN`) for PII written via the `@vercel/blob` SDK directly (certificate PDFs, data exports) and served through short-TTL signed URLs.
