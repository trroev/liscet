...
- **`postUpdateOptions: ["pnpmDedupe"]` is required for our pnpm catalog.** Without it, Renovate can commit a `pnpm-lock.yaml` whose `catalogs:` snapshot isn't fully re-resolved against `pnpm-workspace.yaml`, so CI's `pnpm install --frozen-lockfile` fails with `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH`. Forcing a `pnpm dedupe` after each update makes Renovate re-resolve the whole lockfile (matching what a local `pnpm install` produces). This first surfaced when the initial security backlog bumped `pnpm` itself (10.33.2 → 10.34.5) across stacked branches.
